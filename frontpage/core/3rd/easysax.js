'use strict';
// https://raw.githubusercontent.com/vflash/easysax/master/easysax.js
// MIT License
/*
new function() {
    ...
    parser.on('startNode', function(elemName, getAttr, isTagEnd, getStrNode) {
        var attr = getAttr();
	// elementName-（string）元素名称。指定名称空间时，前缀
	// getAttr（）-（function）解析属性并返回对象。
    // isTagEnd-（布尔值）标志元素为空"<elem/>"
	// getStringNode（）-（函数）返回元素的未分析的字符串示例：<item title="text" id="x345">
    });
...
window.EasySAXParser = EasySAXParser;
